...
const renderResult = (result, isAssigned = false) => {
  ...
  // Always use 'default' badge style for null/infinity jumps
  const badgeStyle = getBadgeStyle(
    result.jumps === null || result.jumps === Infinity ? 'default' : result.colorCode
  );
      ...
      <div style={styles.resultHeader}>
        ...
        <div style={styles.badgeContainer}>
          <span style={badgeStyle}>
            {/* Always append "jumps" to the number */}
            {result.jumps === null || result.jumps === Infinity ? '0 jumps' : `${result.jumps} jumps`}
          </span>
        </div>
      </div>
