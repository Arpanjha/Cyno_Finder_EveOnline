...
const styles = {
  ...
  cardsSection: {
    ...
    flexDirection: 'column',
    alignItems: 'stretch',
    paddingBottom: '24px', // Add padding at bottom for scrolling
  },
  cardsContainer: {
    display: 'grid',
    ...
    gridAutoRows: 'min-content', // Let cards take their natural height
  },
  errorMessage: {
    position: 'fixed',
    ...
    setCards(prevCards => {
      ...
      return prevCards.map(card => {
        ...
        if (newPriority < oldPriority) {
          if (card.priority >= newPriority && card.priority < oldPriority) {
            return { ...card, priority: card.priority + 1 };
          }
