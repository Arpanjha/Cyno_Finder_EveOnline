...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const system of sortedSystems) {
      ...
      const assignedResults = await Promise.all(allRecords
        .filter(c => 
        ...
        .map(async c => {
          ...
          // Calculate jumps for assigned characters
          let jumps = Infinity;
          try {
            const routeUrl = `https://esi.evetech.net/latest/route/${c.solar_system_id}/${assignment.systemId}/`;
            const routeRes = await fetch(routeUrl);
            
            if (routeRes.ok) {
              const routeData = await routeRes.json();
              jumps = routeData.length > 1 ? routeData.length - 1 : 0;
            }
          } catch (error) {
            console.error(`Route fetch failed for assigned cyno ${c.character_id}:`, error);
          }
          ...
          return {
            ...
            jumps, // Now using actual calculated jumps
            characterId: c.character_id,
            colorCode: 'default',  // Keep the grey styling
            assignedTo: {
